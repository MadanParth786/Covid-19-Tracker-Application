...
# setting title
root.title("Get Covid-19 Details")
...
Label(root, text="Enter Countries Name\nTo get\ncovid-19 data", font="Consolas 15 bold").pack()
Label(root, text="Enter country name:").pack()
data = StringVar()
data.set("Enter Detail")
entry = Entry(root, textvariable=data, width=50).pack()
Button(root, text="Get Data", command=showdata).pack()
Label(root, text="The Given Results are As Shown ",font="Consolas 15 bold").pack()
Label(root, text="****Wear Mask Stay Safe****",font="Consolas 15 bold").pack()
root.mainloop()
